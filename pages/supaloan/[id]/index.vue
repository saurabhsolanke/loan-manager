  ...
  <div class="min-h-screen bg-gray-50">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center py-4 sm:py-6 space-y-4 sm:space-y-0">
          ...
          <div class="flex items-center space-x-4">
            <button @click="refreshLoan" :disabled="loading"
              ...
              <svg v-if="loading" class="animate-spin -ml-1 mr-2 sm:mr-3 h-4 w-4 sm:h-5 sm:w-5 text-white"
    ...
    <div v-else-if="loan" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <div class="mb-4 sm:mb-6">
        <div :class="getStatusClass(loan)" class="rounded-md p-3 sm:p-4">
          <div class="flex">
            <div class="flex-shrink-0">
              <svg v-if="getStatusText(loan) === 'Overdue'" class="h-4 w-4 sm:h-5 sm:w-5 text-red-500"
                viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd"
                  d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
                  clip-rule="evenodd" />
              </svg>
              <svg v-else-if="getStatusText(loan) === 'Active'" class="h-4 w-4 sm:h-5 sm:w-5 text-blue-500"
                viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd"
            ...
            <div class="ml-3">
              <h3 class="text-xs sm:text-sm font-medium"
                :class="getStatusText(loan) === 'Overdue' ? 'text-red-700' : getStatusText(loan) === 'Active' ? 'text-blue-700' : 'text-green-700'">
                {{ getStatusText(loan) }} - {{ loan.BranchName }}
              </h3>
              <div class="mt-1 text-xs sm:text-sm"
                :class="getStatusText(loan) === 'Overdue' ? 'text-red-600' : getStatusText(loan) === 'Active' ? 'text-blue-600' : 'text-green-600'">
        ...
        <div class="xl:col-span-1">
          <div class="bg-white shadow rounded-lg border border-gray-200">
            <div class="px-4 py-4 sm:py-5 sm:p-6">
              <h3 class="text-base sm:text-lg leading-6 font-medium text-gray-900 mb-3 sm:mb-4">Borrower Information
              </h3>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div class="space-y-3 sm:space-y-4">
                  ...
                  <div>
                    ...
                    <dd class="mt-1 text-base sm:text-lg font-semibold text-gray-900">
                      ₹{{ formatCurrency(loan['Loan Amount']) }}</dd>
                  </div>
                  <div>
                ...
                <div class="space-y-3 sm:space-y-4">
                  <div>
                    ...
                    <dd class="mt-1 text-base sm:text-lg font-semibold text-red-600">
                      ₹{{ formatCurrency(loan['Total Overdue Amount']) }}</dd>
                  </div>
                  <div>
                    <dt class="text-xs sm:text-sm font-medium text-gray-500">Total Recovery Amount</dt>
                    <dd class="mt-1 text-sm sm:text-base text-gray-900">₹{{ formatCurrency(loan['Total Recovery Amount']
                      || 0) }}</dd>
                  </div>
                  <div>
                    <dt class="text-xs sm:text-sm font-medium text-gray-500">Installment Amount</dt>
                    <dd class="mt-1 text-sm sm:text-base text-gray-900">₹{{ formatCurrency(loan['Installment Amount'])
                    }}</dd>
                  </div>
                  <div>
      ...
      <div class="mt-4 sm:mt-6">
        <div class="bg-white shadow rounded-lg border border-gray-200">
          <div class="px-4 py-4 sm:py-5 sm:p-6">
            <h3 class="text-base sm:text-lg leading-6 font-medium text-gray-900 mb-3 sm:mb-4">Additional Information
            </h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              <div>
    ...
    <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      <div class="text-center">
        ...
        <p class="mt-1 text-xs sm:text-sm text-gray-500">The loan with account number {{ route.params.id }} could not be
          found.</p>
        <div class="mt-4 sm:mt-6">
          <NuxtLink to="/supaloan"

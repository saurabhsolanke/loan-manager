      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center py-4 sm:py-6 space-y-4 sm:space-y-0">
          <div class="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
            <NuxtLink 
              to="/supaloan" 
              class="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors w-fit"
            >
          ...
          <div class="flex items-center space-x-4">
            <button
              @click="refreshLoan"
              :disabled="loading"
              class="inline-flex items-center px-3 sm:px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 transition-colors"
            >
    ...
    <div v-else-if="loan" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <div class="mb-4 sm:mb-6">
        <div :class="getStatusClass(loan)" class="rounded-md p-3 sm:p-4">
          <div class="flex">
            <div class="flex-shrink-0">
              <svg v-if="getStatusText(loan) === 'Overdue'" class="h-4 w-4 sm:h-5 sm:w-5 text-red-400" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
              </svg>
              <svg v-else-if="getStatusText(loan) === 'Active'" class="h-4 w-4 sm:h-5 sm:w-5 text-yellow-400" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
              </svg>
              <svg v-else class="h-4 w-4 sm:h-5 sm:w-5 text-green-400" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div class="ml-3">
              <h3 class="text-xs sm:text-sm font-medium" :class="getStatusClass(loan).replace('bg-', 'text-')">
                {{ getStatusText(loan) }} - {{ loan.BranchName }}
              </h3>
              <div class="mt-1 text-xs sm:text-sm" :class="getStatusClass(loan).replace('bg-', 'text-').replace('100', '700')">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div class="space-y-3 sm:space-y-4">
                  ...
                  <div>
                    ...
                    <dd class="mt-1 text-base sm:text-lg font-semibold" :class="loan['Closing Balance'] < 0 ? 'text-red-600' : 'text-green-600'">
    ...
    <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      <div class="text-center">
        ...
        <div class="mt-4 sm:mt-6">
          <NuxtLink 
            to="/supaloan" 
            class="inline-flex items-center px-3 sm:px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700"
          >
            Back to Loans
          </NuxtLink>
        </div>
